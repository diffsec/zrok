...
func (s *orgStore) Create(ctx context.Context, o *store.Org) error {
	...
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO organizations(id,name,github_login,created_at,updated_at) VALUES(?,?,?,?,?)`,
		o.ID, o.Name, o.GitHubLogin, o.CreatedAt, o.UpdatedAt)
	return err
}

func (s *orgStore) Get(ctx context.Context, id string) (*store.Org, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id,name,github_login,created_at,updated_at FROM organizations WHERE id=?`, id)
	o := &store.Org{}
...
func (s *orgStore) GetByGitHubLogin(ctx context.Context, login string) (*store.Org, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id,name,github_login,created_at,updated_at FROM organizations WHERE github_login=?`, login)
	o := &store.Org{}
	if err := row.Scan(&o.ID, &o.Name, &o.GitHubLogin, &o.CreatedAt, &o.UpdatedAt); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, store.ErrNotFound
...
func (s *orgStore) Update(ctx context.Context, o *store.Org) error {
	...
	_, err := s.db.ExecContext(ctx,
		`UPDATE organizations SET name=?,github_login=?,updated_at=? WHERE id=?`,
		o.Name, o.GitHubLogin, o.UpdatedAt, o.ID)
	return err
}

func (s *orgStore) List(ctx context.Context) ([]*store.Org, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id,name,github_login,created_at,updated_at FROM organizations ORDER BY created_at`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
