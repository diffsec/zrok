...
func (s *findingActionStore) Append(ctx context.Context, findingID, action, actorID, reason, payloadJSON string) error {
	...
	_, err = s.db.ExecContext(ctx,
		`INSERT INTO finding_actions(id,finding_id,action,actor_id,reason,payload_json,created_at)
		 VALUES(?,?,?,?,?,?,?)`,
		id.String(), findingID, action, nullStr(actorID), nullStr(reason), nullStr(payloadJSON),
		time.Now().UTC())
	return err
}
...
func (s *findingActionStore) List(ctx context.Context, findingID string) ([]*store.FindingAction, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id,finding_id,action,COALESCE(actor_id,''),COALESCE(reason,''),
		    COALESCE(payload_json,''),created_at
		 FROM finding_actions WHERE finding_id=? ORDER BY created_at`, findingID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
