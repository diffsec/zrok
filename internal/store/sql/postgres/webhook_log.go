...
func (s *webhookLogStore) Append(ctx context.Context, w *store.GitHubWebhookLog) error {
	...
	_, err := s.db.ExecContext(ctx,
		...
		w.ID, w.DeliveryID, w.EventType, nullStr(w.Action), nullJSON(w.PayloadJSON),
...
func (s *webhookLogStore) Get(ctx context.Context, deliveryID string) (*store.GitHubWebhookLog, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id,delivery_id,event_type,COALESCE(action,''),COALESCE(payload_json::text,''),COALESCE(signature,''),received_at,processed_at,COALESCE(process_error,'')
		 FROM github_webhook_log WHERE delivery_id=$1`, deliveryID)
...
func (s *webhookLogStore) MarkProcessed(ctx context.Context, deliveryID, processError string) error {
	_, err := s.db.ExecContext(ctx,
		`UPDATE github_webhook_log SET processed_at=$1, process_error=$2 WHERE delivery_id=$3`,
