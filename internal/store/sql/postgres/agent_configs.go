...
func (s *agentConfigStore) Create(ctx context.Context, ac *store.AgentConfigRow) error {
	...
	_, err := s.db.ExecContext(ctx,
		...
		nullUUID(ac.ActiveRevisionID), ac.CreatedAt, ac.UpdatedAt)
	return err
}

func (s *agentConfigStore) Get(ctx context.Context, id string) (*store.AgentConfigRow, error) {
	row := s.db.QueryRowContext(ctx, agentConfigSelect+` WHERE id=$1`, id)
...
func (s *agentConfigStore) GetByName(ctx context.Context, orgID, name string) (*store.AgentConfigRow, error) {
	row := s.db.QueryRowContext(ctx, agentConfigSelect+` WHERE org_id=$1 AND name=$2`, orgID, name)
	return scanAgentConfig(row.Scan)
}

func (s *agentConfigStore) List(ctx context.Context, orgID string) ([]*store.AgentConfigRow, error) {
	rows, err := s.db.QueryContext(ctx, agentConfigSelect+` WHERE org_id=$1 ORDER BY name`, orgID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *agentConfigStore) NewRevision(ctx context.Context, rev *store.AgentConfigRevision) error {
	...
	if rev.Revision == 0 {
		row := s.db.QueryRowContext(ctx,
			`SELECT COALESCE(MAX(revision),0)+1 FROM agent_config_revisions WHERE agent_config_id=$1`,
			rev.AgentConfigID)
	...
	_, err := s.db.ExecContext(ctx,
		...
		nullJSON(rev.ModelConfigJSON), nullStr(rev.ImportSHA),
		nullUUID(rev.CreatedBy), rev.CreatedAt)
	return err
}

func (s *agentConfigStore) ListRevisions(ctx context.Context, agentConfigID string) ([]*store.AgentConfigRevision, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id,agent_config_id,revision,yaml,COALESCE(model_config_json::text,''),COALESCE(import_sha,''),COALESCE(created_by::text,''),created_at
		 FROM agent_config_revisions WHERE agent_config_id=$1 ORDER BY revision DESC`,
		agentConfigID)
	if err != nil {
		return nil, err
	}
...
func (s *agentConfigStore) SetActiveRevision(ctx context.Context, agentConfigID, revisionID string) error {
	_, err := s.db.ExecContext(ctx,
		`UPDATE agent_configs SET active_revision_id=$1, updated_at=$2 WHERE id=$3`,
		revisionID, time.Now().UTC(), agentConfigID)
	return err
}

const agentConfigSelect = `SELECT id,org_id,name,COALESCE(description,''),phase,source,COALESCE(active_revision_id::text,''),created_at,updated_at FROM agent_configs`
