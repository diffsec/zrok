...
func (s *transcriptStore) Put(ctx context.Context, runID, invocationID, agentName, storageURI, sha256 string, byteSize int64) error {
	...
	_, err = s.db.ExecContext(ctx,
		...
		id.String(), runID, nullUUID(invocationID), agentName, storageURI, byteSize, nullStr(sha256), time.Now().UTC())
	return err
}

func (s *transcriptStore) List(ctx context.Context, runID string) ([]*store.Transcript, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id,run_id,COALESCE(invocation_id::text,''),agent_name,storage_uri,byte_size,COALESCE(sha256,''),created_at
		 FROM transcripts WHERE run_id=$1 ORDER BY created_at`, runID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
