...
func (s *runStore) GetByRepoAndHead(ctx context.Context, repoID, headSHA string) (*store.Run, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT `+runSelectColumns+` FROM runs WHERE repo_id=$1 AND head_sha=$2 ORDER BY created_at DESC LIMIT 1`,
		repoID, headSHA)
	return scanRun(row)
}
...
func (s *runStore) UpdateError(ctx context.Context, id, category, message string) error {
	_, err := s.db.ExecContext(ctx,
		`UPDATE runs SET error_category=$1, error_message=$2 WHERE id=$3`,
		nullStr(category), nullStr(message), id)
	return err
}

func (s *runStore) MarkStarted(ctx context.Context, id string, t time.Time) error {
	_, err := s.db.ExecContext(ctx, `UPDATE runs SET started_at=$1 WHERE id=$2 AND started_at IS NULL`, t, id)
	return err
}

func (s *runStore) MarkCompleted(ctx context.Context, id, status string, t time.Time) error {
	_, err := s.db.ExecContext(ctx, `UPDATE runs SET status=$1, completed_at=$2 WHERE id=$3`, status, t, id)
	return err
}

func (s *runStore) Cancel(ctx context.Context, id string) error {
	_, err := s.db.ExecContext(ctx,
		`UPDATE runs SET status='cancelled', completed_at=$1 WHERE id=$2 AND status NOT IN ('completed','failed','cancelled')`,
		time.Now().UTC(), id)
	return err
}
