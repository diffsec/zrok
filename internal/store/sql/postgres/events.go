...
func (s *eventStore) Append(ctx context.Context, runID, eventType, payloadJSON string) error {
	...
	_, err = s.db.ExecContext(ctx,
		`INSERT INTO timeline_events(id,run_id,ts,event_type,payload_json) VALUES($1,$2,$3,$4,$5)`,
		id.String(), runID, time.Now().UTC(), eventType, nullJSON(payloadJSON))
	return err
}

func (s *eventStore) Stream(ctx context.Context, runID string, sinceTS time.Time) ([]*store.TimelineEvent, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id,run_id,ts,event_type,COALESCE(payload_json::text,'') FROM timeline_events
		 WHERE run_id=$1 AND ts>=$2 ORDER BY ts, id`,
		runID, sinceTS)
	if err != nil {
		return nil, err
	}
